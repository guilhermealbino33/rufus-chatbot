...
async function runHealthCheck() {
  logger.log({
    ...
    message: `WPPConnect Browser Health Check | timeout=${SCRIPT_TIMEOUT_MS / 1000}s | ${new Date().toISOString()}`,
  });
  ...
  logger.log({
    severity: LogSeverity.LOG,
    message: `[1/5] Chromium encontrado: ${executablePath}`,
  });
  ...
  try {
    ...
    logger.log({
      severity: LogSeverity.LOG,
      message: `[2/5] Chromium: ${version}`,
    });
  } catch (e) {
    logger.warn({
      severity: LogSeverity.WARNING,
      message: `Não foi possível obter a versão: ${e.message}`,
    });
  }
  ...
  try {
    ...
    logger.log({
      severity: LogSeverity.LOG,
      message: '[3/5] WPPConnect criado com sucesso!',
    });
    ...
    logger.log({
      severity: LogSeverity.LOG,
      message: `[4/5] Página carregada: "${pageTitle}" (${WHATSAPP_WEB_URL})`,
    });
    ...
    logger.log({
      severity: LogSeverity.LOG,
      message: `[5/5] Screenshot salvo: ${screenshotPath}`,
    });
