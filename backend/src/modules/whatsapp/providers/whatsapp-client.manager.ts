...
export class WhatsappClientManager implements OnModuleDestroy {
    ...
    async createClient(
        ...
        if (this.clients.has(sessionName)) {
            this.logger.warn(
                `⚠️ Client for ${sessionName} already exists. Returning existing instance.`
            );
            return this.clients.get(sessionName)!;
        }
    ...
    async removeClient(sessionName: string): Promise<void> {
        ...
        try {
            ...
            this.logger.log(`🗑️ Client removed from memory: ${sessionName} (Remaining: ${this.clients.size})`);
        }
    }
    ...
    async isClientConnected(sessionName: string): Promise<boolean> {
        ...
        try {
            ...
            this.logger.error(
                `❌ Error checking connection for ${sessionName}:`,
                error.message
            );
    ...
    async getConnectionState(sessionName: string): Promise<string | null> {
        ...
        try {
            ...
            this.logger.error(
                `❌ Error getting connection state for ${sessionName}:`,
                error.message
            );
            return null;
        }
    ...
    async closeAll(): Promise<void> {
        ...
        this.logger.log(
            `✅ Cleanup completed: ${success} closed successfully, ${failed} failed`
        );
    }
