...
import { WhatsappClientConfig, DEFAULT_WHATSAPP_CONFIG } from '../../../crosscutting/config/whatsapp-client.config';
...
export class WhatsappClientFactory {
    ...
    async create(config: WhatsappClientConfig): Promise<wppconnect.Whatsapp> {
        ...
        try {
            ...
            throw new InternalServerErrorException(
                `Failed to initialize WPPConnect: ${error.message}`
            );
        }
    }
     ...
     */
    private buildWppConnectOptions(config: WhatsappClientConfig): any {
        const options: any = {
